...
f = open (os.path.join ("sharez.in"), "rt")
data = f.read ()
f.close ()
data = data.replace ("@prefix@", prefix)
data = data.replace ("@version@", version)
f = open (os.path.join ("sharez"), "wt")
f.write (data)
f.close ()

#cmd = "intltool-merge -d -u po/ sharez.desktop.in sharez.desktop".split(" ")
...
local_icon_path = "share/sharez/icons/hicolor/"
...
if os.path.isdir (podir):
    buildcmd = "msgfmt -o build/locale/%s/sharez.mo po/%s.po"
    mopath = "build/locale/%s/sharez.mo"
...
setup (
        name             = "sharez",
        version          = version,
        description      = "Share local data",
        author           = "Patrick Niklaus",
        author_email     = "marex@compiz-fusion.org.org",
        url              = "http://github.com/TheMarex/Sharez",
        license          = "GPL",
